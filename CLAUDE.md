...
- `listening/` -- 英語リスニングクイズ（1,099問・5 axis・4モード・seenSet・SRS・マイルストーン・パーソナライズCTA）
...
- 1,099問（2026-03-15時点、diff lv1〜lv5 × axis 5軸）
- 700+ MP3ファイル（`audio/q001.mp3`〜）、5種音声ローテーション（AriaNeural/SoniaNeural/GuyNeural/NatashaNeural/RyanNeural）
- **適応型難易度アルゴリズム**（2026-02-24実装）:
  - intermediate（中級）からスタート
  - 連続2問正解 → 1段階上へ
  - 1問不正解 → 1段階下へ（即時）、不正解問題は新しいレベルのプールに再挿入
  - 難易度ラベルはユーザーに非表示
- **seenSet（既出管理）**（2026-03-15実装）:
  - localStorageキー `listenup_seen` で回答済み問題IDを追跡
  - 未回答問題を優先出題、全問消化でリセット
  - `loadSeen()` / `saveSeen()` / `markSeen(q)` で管理
- **4つのクイズモード**（2026-03-15実装）:
  - `normal`: 全問題から出題（デフォルト）
  - `daily`: 曜日別axis（月=speed, 火=reduction, 水=vocab, 木=context, 金=distractor, 土日=全体）
  - `weak`: 弱点axis（正解率下位2つ）から70%出題
  - `revenge`: SRS登録済み + wrongOnce の問題のみ
...
## listening/ 問題配分・作成方針（2026-02-28確定）

### 現在の問題配分（1,099問・2026-03-15時点）

#### diff（難易度）分布

| レベル | 基準 | 問題数 | 目標割合 |
|---|---|---|---|
| lv1 | ≤13語・基本語彙・1回聴けば分かる | ~110問 | 15% |
| lv2 | 14-17語・日常語彙・イディオム≤1 | ~183問 | 25% |
| lv3 | 18-21語・中級語彙・イディオム1-2 | ~220問 | 30% |
| lv4 | 22-26語・スラング・句動詞・ニュアンス | ~146問 | 20% |
| lv5 | ≥27語 or 上級表現・文脈依存・3回以上 | ~73問 | 10% |

#### axis（難しさの質）分布（2026-03-15時点）

| axis | 問題数 | 割合 | 状態 |
|---|---|---|---|
| speed | 188問 | 17.1% | ✅ 目標水準 |
| reduction | 195問 | 17.7% | ✅ 目標水準 |
| vocab | 194問 | 17.7% | ✅ 目標水準 |
| context | 284問 | 25.8% | ⚠️ やや過多（薄まり中） |
| distractor | 244問 | 22.2% | ✅ 補強済み（+100問 distractor特化生成） |

**補足**: 2026-03-15にdistractor特化で100問追加。context過多は新規均等生成で自然に薄まる。

#### 音声ボイス分布

| ボイス | 特徴 | 問題数 |
|---|---|---|
| en-US-AriaNeural | 米国英語・女性 | 147問 (20.1%) |
| en-GB-SoniaNeural | 英国英語・女性 | 147問 (20.1%) |
| en-US-GuyNeural | 米国英語・男性 | 146問 (19.9%) |
| en-AU-NatashaNeural | 豪州英語・女性 | 146問 (19.9%) |
| en-GB-RyanNeural | 英国英語・男性 | 146問 (19.9%) |

インデックス連番の5ローテーションで自動均等配分。追加しても崩れない。

### 問題作成方針

#### 基本原則
- **ネイティブが実際に使う自然な英語**（略語・短縮形OK）
- **1文完結**が基本。長くなるのは lv4/lv5 のみ
- **音声は正解確定に必要な情報を含む**（選択肢だけで解けてはいけない）
- **5択**：正解1 + 紛らわしい誤答4。正解を先頭にしない

#### axis ごとの作り方

| axis | text に求めること | choices に求めること |
|---|---|---|
| speed | "Didja" "Gonna hafta" "Whadya" 等の崩れた発音を文字化 | 聞き取れた語句から誤解しやすい選択肢 |
| reduction | gonna/wanna/kinda/dunno/lemme/hafta/s'posed to 等を含める | 音変化なしで読んだ場合の誤解選択肢 |
| vocab | イディオム・スラング・比喩（pull it off / hit the sack 等） | 語句を知らないと選びがちな直訳的誤答 |
| context | 語句は平易だが状況・トーン・感情から意図を推論する必要がある | 語句だけ拾うと選んでしまう表面的誤答 |
| distractor | 語句も文脈も理解できるが選択肢が紛らわしい | 特に正解と1語違い・同シチュエーション違う行動 |

#### 今後の補強優先度
1. **distractor**: 74問（10%）→ 100問超を目標。次回 `--axis-only distractor` で補強
2. **context**: 新規生成時は均等分散なので自然に割合が下がる
3. **lv5**: 73問（10%）→ 維持。無理に増やさない
...
## listening/ のプラン設計（2026-03-15更新）

### 運用方針
- **メアド登録は撤去済み**（2026-03-15）
- **問数制限なし**（以前の5問/15問制限は撤去済み）
