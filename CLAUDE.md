...
#### axis（難しさの質）分布（2026-02-28時点）

| axis | 問題数 | 割合 | 状態 |
|---|---|---|---|
| speed | 134問 | 18.3% | ✅ 目標水準 |
| reduction | 142問 | 19.4% | ✅ 目標水準 |
| vocab | 140問 | 19.1% | ✅ 目標水準 |
| context | 242問 | 33.1% | ⚠️ やや過多（今後新規生成で薄める） |
| distractor | 74問 | 10.1% | 不足気味（補強余地あり） |

**補足**: context 過多は既存問題が axis 設計前に生成されたため。新規生成時は均等分散指示あり。
...
### プランについて
- **有料プランは廃止**（¥490/月 の Waitlist 方式は中止）
- 無料（未登録 5問/日）+ メール登録（15問/日）の2段階のみ
